...
template <size_t nparts, size_t dimx, size_t dimy, typename resampT, typename float_t>
svol_bs<nparts, dimx, dimy, resampT, float_t>::svol_bs(
  const float_t & phi, const float_t & beta, const float_t & sigma)
...
float_t svol_bs<nparts, dimx, dimy, resampT, float_t>::logQ1Ev(const ssv & x1samp, const osv & y1)
{
  return rveval::evalUnivNorm<float_t>(
    x1samp(0), 0.0, m_sigma / std::sqrt(1.0 - m_phi * m_phi), true);
}

template <size_t nparts, size_t dimx, size_t dimy, typename resampT, typename float_t>
auto svol_bs<nparts, dimx, dimy, resampT, float_t>::get_uwtd_samps() const
  -> std::array<ssv, nparts>
{
